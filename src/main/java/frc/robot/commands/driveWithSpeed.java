...
public class driveWithSpeed extends Command {
  ...
  @Override
  public void execute() {
    drivetrain.setControl(drive.withVelocityX(-joystick.getLeftY() * MaxSpeed * tempGovernor) // Drive forward with
                                                                                              // negative Y (forward)
        .withVelocityY(-joystick.getLeftX() * MaxSpeed * tempGovernor) // Drive left with negative X (left)
        .withRotationalRate(-joystick.getRightX() * MaxAngularRate * tempGovernor)); // Drive counterclockwise with
                                                                                     // negative X (left)
  }
